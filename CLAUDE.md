...
Tests: `PYTHONPATH=src pytest -q` (pyproject.toml also sets `pythonpath = ["src"]`, so plain `pytest -q` from the
  repo root works once the venv is active). Healthy collection as of the last run: 114 tests, all green.
...
Package layout: `src/ai_check_bot/` (src-layout, not a flat package) — `config.py`, `db.py`/`models.py`,
  ...
  `chek_protocol_text.py` (extracts step sections/prompts from CHEK_PROTOCOL.md at runtime, never duplicated as a
  second copy) + `chek_fleet.py` (CHEK Step 5: fleet planner, parses its DOMAIN/PROMPT/SUMMARY output),
