...
def test_parses_valid_lines():
    text = (
        "critical|AutoPost|app/auth.py::validate_token|Токен не проверяется на None\n"
        "high|AutoPost|app/db.py::save|Не закрывается соединение\n"
    )
    parsed = parse_structured_findings(text)
    assert len(parsed) == 2
    assert parsed[0].severity == "critical"
    assert parsed[0].project_name == "AutoPost"
...
def test_skips_malformed_lines():
    text = (
        "critical|AutoPost|app/auth.py::validate_token|описание\n"
